...
export function StaffCoursesView() {
  ...
  const availableSkills = ["Computer Science", "Algorithms", "Networking", "Security", "C Programming", "Logic", "Java", "OOP", "Database Design", "React.js", "Python"];
  const availableCourses = courses.filter(c => c.is_active).map(c => c.courseCode);

  // Filter Options
  const availableCreditsList = Array.from(new Set(courses.map(c => c.credits))).sort((a, b) => a - b);
  const availableSkillsList = Array.from(new Set(courses.flatMap(c => c.skills))).sort();
  ...
  const handleOpenSingleEdit = (course: CourseMockData) => {
    setEditMode('single');
  ...
  const handleOpenBulkEdit = () => {
    ...
    // Initialize with first selected course's editable data
    ...
    if (firstCourse) {
      setFormData({
        ...
        prerequisites: firstCourse.prerequisites ? [...firstCourse.prerequisites] : []
      });
    }
  };
  ...
  const handleSaveEdit = () => {
    setCourses(prev => prev.map(course => {
      if (editMode === 'single' && course.courseId === editingCourseId) {
        return {
          ...course,
      ...
      } else if (editMode === 'bulk' && selectedCourseIds.includes(course.courseId)) {
        return {
          ...course,
          credits: formData.credits,
          totalStudyHours: formData.totalStudyHours,
  ...
  const confirmDelete = () => {
    setCourses(prev => prev.map(course => {
      if (deleteType === 'single' && course.courseId === deleteCourseId) {
        return { ...course, is_active: false };
      } else if (deleteType === 'bulk' && selectedCourseIds.includes(course.courseId)) {
        return { ...course, is_active: false };
      }
      return course;
    }));
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
            <div>
              ...
              <h1 className="text-[24px] font-bold tracking-tight text-[#0F172A] mb-1">
                Courses Directory ({activeCourses.length})
              </h1>
              ...
              <table className="min-w-full divide-y divide-gray-100 border-collapse">
                <thead>
                  <tr className="bg-[#F8FAFC]">
                    ...
                    <th className="px-6 py-4 text-left text-[11px] font-bold text-[#64748B] uppercase tracking-wider">Course Code</th>
                    <th className="px-6 py-4 text-left text-[11px] font-bold text-[#64748B] uppercase tracking-wider">Course Name</th>
                    <th className="px-6 py-4 text-left text-[11px] font-bold text-[#64748B] uppercase tracking-wider">Prerequisites</th>
                    <th className="px-6 py-4 text-center text-[11px] font-bold text-[#64748B] uppercase tracking-wider">Credits</th>
                    <th className="px-6 py-4 text-center text-[11px] font-bold text-[#64748B] uppercase tracking-wider">Study Hours</th>
                    <th className="px-6 py-4 text-left text-[11px] font-bold text-[#64748B] uppercase tracking-wider">Core Skills</th>
                    {isGlobalEditMode && (
                  ...
                  {paginatedCourses.length > 0 ? (
                    paginatedCourses.map((course) => (
                      <tr key={course.courseId} className="hover:bg-[#F8FAFC]/50 transition-colors">
                        ...
                        <td className="px-6 py-4 text-[12px] font-semibold text-[#64748B]">
                          {course.prerequisites && course.prerequisites.length > 0 ? course.prerequisites.join(", ") : "-"}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-[13px] text-center font-semibold text-[#334155]">
                          {course.credits}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-[13px] text-center font-semibold text-[#334155]">
                          {course.totalStudyHours}
                        </td>
